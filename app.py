...
TR = {
    "en": {
        ...
        "cat_summary_sub":"Total items, records, and average count per record for each trash category.",
...
st.set_page_config(page_title="SCR Trash Survey · Sonoran Institute", page_icon="https://sonoraninstitute.org/wp-content/uploads/2018/02/cropped-si_icon_2018-32x32.png",
...
.sec-sub{{font-size:12.5px;color:{C["muted"]};margin-bottom:24px;line-height:1.85;}}
@media print {{
...
.tbl-note{{font-size:12px;color:{C["muted"]};line-height:1.85;padding:16px 0 8px;
...
.card{{background:#fff;border:1px solid {C["sand3"]};border-radius:12px;
       padding:36px 36px 28px;margin-bottom:36px;box-shadow:0 2px 14px rgba(0,0,0,.05);}}
...
/* Elements start hidden, reveal on scroll */
...
.reveal-scale{{opacity:0;transform:scale(.95);transition:opacity .55s cubic-bezier(.16,1,.3,1),transform .55s cubic-bezier(.16,1,.3,1);}}
.reveal-scale.visible{{opacity:1;transform:none;}}
.reveal-left{{opacity:0;transform:translateX(-20px);transition:opacity .5s cubic-bezier(.16,1,.3,1),transform .5s cubic-bezier(.16,1,.3,1);}}
.reveal-left.visible{{opacity:1;transform:none;}}

/* Stagger children — each child delays slightly */
.reveal-stagger>.kpi,.reveal-stagger>.stat-item{{opacity:0;transform:translateY(16px);
    transition:opacity .5s cubic-bezier(.16,1,.3,1),transform .5s cubic-bezier(.16,1,.3,1);}}
.reveal-stagger.visible>.kpi:nth-child(1),.reveal-stagger.visible>.stat-item:nth-child(1){{opacity:1;transform:none;transition-delay:.06s;}}
.reveal-stagger.visible>.kpi:nth-child(2),.reveal-stagger.visible>.stat-item:nth-child(2){{opacity:1;transform:none;transition-delay:.12s;}}
.reveal-stagger.visible>.kpi:nth-child(3),.reveal-stagger.visible>.stat-item:nth-child(3){{opacity:1;transform:none;transition-delay:.18s;}}
.reveal-stagger.visible>.kpi:nth-child(4),.reveal-stagger.visible>.stat-item:nth-child(4){{opacity:1;transform:none;transition-delay:.24s;}}
.reveal-stagger.visible>.kpi:nth-child(5){{opacity:1;transform:none;transition-delay:.30s;}}

/* Page entrance — only the first body fade */
.fade-up{{animation:fadeUp .55s cubic-bezier(.16,1,.3,1) both;}}

/* Card hover lift */
.card{{transition:box-shadow .35s cubic-bezier(.16,1,.3,1),transform .35s cubic-bezier(.16,1,.3,1);}}
...
/* Buttons */
.stButton>button,.stDownloadButton>button{{transition:all .3s cubic-bezier(.16,1,.3,1)!important;}}
.stButton>button:hover{{transform:translateY(-1px)!important;}}

/* Nav tabs */
div[role="radiogroup"] > label {{transition:color .25s ease,border-color .3s ease,background .25s ease !important;}}

/* Charts and tables — scroll-triggered via JS */
div[data-testid="stPlotlyChart"],div[data-testid="stDataFrame"]{{opacity:0;transform:translateY(20px);
    transition:opacity .6s cubic-bezier(.16,1,.3,1),transform .6s cubic-bezier(.16,1,.3,1);}}
div[data-testid="stPlotlyChart"].visible,div[data-testid="stDataFrame"].visible{{opacity:1;transform:none;}}

/* Footer */
.ftr-a{{transition:color .25s ease,transform .2s ease;}}
...
.sec-hd{{opacity:0;transform:translateX(-16px);transition:opacity .5s ease,transform .5s ease;}}
.sec-hd.visible{{opacity:1;transform:none;}}

/* Print + reduced motion */
@media print{{*{{animation:none!important;transition:none!important;opacity:1!important;transform:none!important;}}}}
@media (prefers-reduced-motion: reduce){{
    *,.reveal,.reveal-scale,.reveal-left,.reveal-stagger>.kpi,.reveal-stagger>.stat-item,
    .sec-hd,div[data-testid="stPlotlyChart"],div[data-testid="stDataFrame"]{{
        opacity:1!important;transform:none!important;transition-duration:0.01s!important;animation-duration:0.01s!important;}}
}}
...
def card_open(title, subtitle=""):
    ...
    st.markdown(f'<div class="card reveal"><div class="card-hd"><div><div class="sec-hd">{title}</div>{sub}</div></div><div style="padding-top:6px">', unsafe_allow_html=True)
...
def section_title(text):
    st.markdown(f'<div style="font-family:Cormorant Garamond,serif;font-size:1.3rem;font-weight:700;color:{C["green"]};margin:48px 0 22px;padding-bottom:14px;border-bottom:2px solid {C["sand3"]};">{text}</div>', unsafe_allow_html=True)
...
def fig_note(what, why, read, extra=""):
    ...
    border-radius:0 8px 8px 0;padding:24px 28px;margin:24px 0 36px;box-shadow:0 2px 8px rgba(0,0,0,.04);">
...
def stat_strip(df_orig, df_f):
    ...
    st.markdown(f"""<div class="stat-strip reveal-stagger">
...
# Sign-out CSS removed

# Sign out via header text query params only
...
if page == "Overview":
    ...
    st.markdown(f"""<div class="kpi-grid reveal-stagger">
...
# ══════════════════════════════════════════════════════════════════
    ...
    st.markdown(
        ...
        'Map colors: <span style="color:#3182ce;font-weight:700;">Blue</span> = lower burden, <span style="color:#f59534;font-weight:700;">Orange</span> and <span style="color:#d64541;font-weight:700;">Red</span> = heavier. Click circles for details.<br><br><strong>How burden is calculated:</strong> Total recorded item count per site determines color. Logarithmic scaling ensures visibility across the range.</div>',
        unsafe_allow_html=True
    )
...
# ══════════════════════════════════════════════════════════════════
    ...
    # Photo slideshow with arrow navigation
    components.html(f"""<!DOCTYPE html><html><head>
    <link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500&display=swap" rel="stylesheet">
    <style>
    *{{margin:0;padding:0;box-sizing:border-box;}}
    body{{font-family:'DM Sans',sans-serif;background:transparent;overflow:hidden;}}
    .gallery{{position:relative;width:100%;height:320px;overflow:hidden;border-radius:12px;}}
    .slides{{display:flex;transition:transform .6s cubic-bezier(.16,1,.3,1);height:100%;}}
    .slide{{min-width:100%;height:100%;position:relative;}}
    .slide img{{width:100%;height:280px;object-fit:cover;border-radius:12px;
        box-shadow:0 4px 20px rgba(0,0,0,.12);}}
    .slide .cap{{font-size:12px;color:#686854;font-style:italic;text-align:center;
        margin-top:10px;line-height:1.5;opacity:0;animation:fadeCaption .4s ease .3s both;}}
    @keyframes fadeCaption{{from{{opacity:0;transform:translateY(6px);}}to{{opacity:1;transform:none;}}}}
    .arr{{position:absolute;top:130px;width:44px;height:44px;border-radius:50%;
        ...
        transition:all .25s cubic-bezier(.16,1,.3,1);font-size:18px;color:#3a3a28;}}
    .arr:hover{{background:#fff;box-shadow:0 4px 20px rgba(0,0,0,.18);transform:scale(1.08);}}
    .arr.left{{left:14px;}}
    .arr.right{{right:14px;}}
    .dots{{display:flex;justify-content:center;gap:8px;margin-top:14px;}}
    .dot{{width:8px;height:8px;border-radius:50%;background:#d8ceba;cursor:pointer;
        transition:all .3s ease;}}
    .dot.active{{background:#93a445;width:24px;border-radius:4px;}}
    </style></head><body>
    <div class="gallery">
        <div class="slides" id="slides">
            <div class="slide"><img src="https://sonoraninstitute.org/files/Hatcher_181106_1751-scaled.jpg"><div class="cap">Annual fish survey, Santa Cruz River &middot; &copy;Bill Hatcher / Sonoran Institute</div></div>
            <div class="slide"><img src="https://sonoraninstitute.org/files/Hatcher_181128_404-1600x900.jpg"><div class="cap">Riparian habitat restoration &middot; &copy;Bill Hatcher / Sonoran Institute</div></div>
            <div class="slide"><img src="https://sonoraninstitute.org/files/BHatch_02042018_1116-1600x900.jpg"><div class="cap">Santa Cruz River, April 2018 &middot; &copy;Bill Hatcher / Sonoran Institute</div></div>
            <div class="slide"><img src="https://sonoraninstitute.org/files/BHatch_02042018_1152-1600x900.jpg"><div class="cap">River corridor near Carmen, AZ &middot; &copy;Bill Hatcher / Sonoran Institute</div></div>
            <div class="slide"><img src="https://sonoraninstitute.org/files/IMG_20190702_115922-1-1600x900.jpg"><div class="cap">Field survey crew, Santa Cruz River corridor, 2019</div></div>
            <div class="slide"><img src="https://sonoraninstitute.org/files/246-Santa-Cruz-River-%C2%A9jeffsmith2014-1600x900.jpg"><div class="cap">Santa Cruz River &middot; &copy;Jeff Smith / Sonoran Institute, 2014</div></div>
        </div>
        <div class="arr left" onclick="go(-1)">&lsaquo;</div>
        <div class="arr right" onclick="go(1)">&rsaquo;</div>
    </div>
    <div class="dots" id="dots"></div>
    <script>
    let idx=0;const total=6;const sl=document.getElementById('slides');
    const dots=document.getElementById('dots');
    for(let i=0;i<total;i++){{let d=document.createElement('div');d.className='dot'+(i===0?' active':'');
    d.onclick=()=>goTo(i);dots.appendChild(d);}}
    function goTo(i){{idx=i;sl.style.transform='translateX(-'+idx*100+'%)';
    document.querySelectorAll('.dot').forEach((d,j)=>d.className='dot'+(j===idx?' active':''));
    document.querySelectorAll('.cap').forEach(c=>{{c.style.animation='none';c.offsetHeight;c.style.animation='fadeCaption .4s ease .3s both';}});}}
    function go(dir){{idx=(idx+dir+total)%total;goTo(idx);}}
    ...
    for col, name, role, desc, color in zip([tc1,tc2,tc3],
        ...
        with col:
            ...
            <div style="font-size:13.5px;color:{C['muted']};line-height:1.9;">{desc}</div>
            </div>""", unsafe_allow_html=True)
...
# ── FOOTER ────────────────────────────────────────────────────────
    ...
    # Inject scroll-triggered animation observer
    components.html("""
    <script>
    (function(){
        ...
        function observe(){
            ...
            const selectors = [
                '.reveal', '.reveal-scale', '.reveal-left', '.reveal-stagger',
                '.sec-hd', '.card', '.kpi-grid', '.stat-strip',
        ...
        // Run immediately and re-observe periodically (Streamlit re-renders)
        observe();
