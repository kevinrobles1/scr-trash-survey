...
/* ── ANIMATIONS: simple, reliable, beautiful ── */
@keyframes fadeUp{{from{{opacity:0;transform:translateY(16px);}}to{{opacity:1;transform:none;}}}}
@keyframes scaleUp{{from{{opacity:0;transform:scale(.97);}}to{{opacity:1;transform:none;}}}}
@keyframes bannerPhoto{{from{{opacity:0;transform:scale(1.03);}}to{{opacity:.18;transform:scale(1);}}}}
...
.kpi:hover{{box-shadow:0 10px 30px rgba(122,143,53,.15),0 4px 12px rgba(0,0,0,.06);transform:translateY(-3px);}}
...
/* Buttons */
.stButton>button,.stDownloadButton>button{{transition:all .25s ease!important;}}
.stButton>button:hover{{transform:translateY(-1px)!important;}}

/* Nav tabs */
div[role="radiogroup"] > label{{transition:color .2s ease,border-color .25s ease,background .2s ease !important;}}
...
def page_banner(eyebrow, title, subtitle, img_url=None, img_alt=""):
    ...
    <div style="
      ...
      <div style="position:absolute;inset:0;
        ...
        opacity:0;border-radius:0 0 16px 16px;animation:bannerPhoto .8s ease .15s both;"></div>
  ...
  <div class="hdr-right">
    <div class="hdr-user">
      ...
      <div style="display:flex;align-items:center;justify-content:flex-end;gap:10px;margin-top:4px;">
        ...
        <span style="color:rgba(255,255,255,.2);font-size:9px;">·</span>
        <span style="{_btn_sty}color:rgba(255,255,255,.65);text-decoration:none;">Sign Out</span>
      </div>
    </div>
  </div>
</div></div>""", unsafe_allow_html=True)

# Working sign-out button in top-right corner via iframe
components.html(f"""
<style>
body{{margin:0;padding:0;background:transparent;overflow:hidden;}}
.so-btn{{position:fixed;top:14px;right:20px;z-index:99999;
...
.so-btn:hover{{color:#fff;background:rgba(185,69,27,.55);border-color:rgba(255,255,255,.35);}}
</style>
<a class="so-btn" href="?signout=1" target="_parent">Sign Out</a>
""", height=0, scrolling=False)

# Sign out — button hidden, redirect handled by components.html above
st.markdown("""<style>
/* Hide the sign-out button completely using every known selector */
...
# ══════════════════════════════════════════════════════════════════
    ...
    st.markdown(
        ...
        'Map colors: <span style="color:#3182ce;font-weight:700;">Blue</span> = lower burden, <span style="color:#f59534;font-weight:700;">Orange</span> and <span style="color:#d64541;font-weight:700;">Red</span> = heavier. Click circles for details.<br><br><strong>How burden is calculated:</strong> Total recorded item count per site. Logarithmic color scale.</div>',
        unsafe_allow_html=True
    )
